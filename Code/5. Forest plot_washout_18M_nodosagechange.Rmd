---
title: "Forest plot for results with washout period"
...
dat.folder = "~/Library/CloudStorage/OneDrive-SharedLibraries-MassGeneralBrigham/Data Repository - [P7] Buprenorphine dosagey/Data/"
result.folder = "~/Library/CloudStorage/OneDrive-SharedLibraries-MassGeneralBrigham/Data Repository - [P7] Buprenorphine dosage/Result/"
```


Read in the multivariable model results
```{r}
result = fread(paste(result.folder, "combined_multivariable_M18_nochangedosage_revision.csv", sep=''))
```


Data cleaning
```{r}
# Select the first nine rows for each group defined by 'Outcome_Period'
dosage <- result %>%
...
Make the forest plot with unseparated outcomes
```{r}
...
ggsave(paste(result.folder,'Dosage_M18_nodosagechange.svg', sep=''), plot=forestplot)
...
Forest plot with separate outcomes
```{r}
...
forest <- ggplot(
  ...
  theme(
    ...
    axis.title.x = element_text(size = 10, hjust = 1.15),
...
ggsave(paste(result.folder,'Dosage_M18_nodosagechange_revision.svg', sep=''), plot=forestplot, width = 8, height = 12)
```
